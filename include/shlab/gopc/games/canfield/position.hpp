...
public:
    ...
    constexpr Position(Reserve value) noexcept
        : kind_(Kind::Reserve) {
        storage_.reserve = value;
    }
    ...
    [[nodiscard]] constexpr bool is() const noexcept {
        ...
        else if constexpr (std::is_same_v<T, Reserve>) {
            return kind_ == Kind::Reserve;
        }
    ...
    [[nodiscard]] constexpr T* get_if() noexcept {
        if constexpr (std::is_same_v<T, Foundation>) {
            ...
            return kind_ == Kind::Reserve ? &storage_.reserve : nullptr;
        }
        else if constexpr (std::is_same_v<T, Tableau>) {
    ...
    friend constexpr bool operator==(const Position& lhs, const Position& rhs) noexcept {
        ...
        case Kind::Reserve:
            return lhs.storage_.reserve == rhs.storage_.reserve;
        case Kind::Tableau:
...
constexpr decltype(auto) visit(const Position& pos, Visitor&& vis) {
    ...
    case Position::Kind::Reserve:
        return vis(pos.get<Reserve>());
